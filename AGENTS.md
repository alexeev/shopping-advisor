...
- The [product vision](ROADMAP.md#product-vision--the-shopping-conversation)
  ...
  planning; R15 controls adaptation; R16 governs reuse and architectural
  review — its capability index, lifecycle records and executable promotion
  gate shipped on 2026-09-18, and the entry of task-born capabilities as
  experiments waits for R15; R14 tests the complete loop. **The rest are planned capabilities, not runtime
...
- Say when a category is not supported instead of working around it. Only
  ...
  ship, and `study capabilities` is the index of what each has earned: its
  lifecycle state, the marketplaces it was measured on, what it declines with
  the case records that prove it, and what nothing establishes. Read it before
  deciding that a category fits; an `experiment` is one study's worth of
  evidence, and a report resting on one says so. Explain what is
